...
import { Activity, RefreshCcw, TrendingUp, Newspaper, ExternalLink, Calculator as CalcIcon } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-[#09090b] text-[#fafafa] flex flex-col font-sans">
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full px-4 sm:px-5 py-6 sm:py-8 space-y-6 sm:space-y-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
            {isLoading && !ratesData ? (
              ...
              ratesData?.rates.map((rate, index) => {
                ...
                return (
                  <div key={rate.id} className={`relative overflow-hidden rounded-3xl bg-gradient-to-br ${bgGradient} border border-white/10 shadow-[0_8px_30px_rgb(0,0,0,0.4)] backdrop-blur-md p-5 sm:p-6 flex flex-col justify-between min-h-[140px] sm:min-h-[160px] group transition-all hover:scale-[1.02] hover:border-white/20 touch-manipulation`}>
                    ...
                    <div className="relative z-10 flex items-baseline gap-1 mt-auto">
                      ...
                      <span className="text-base sm:text-lg text-zinc-400 font-bold ml-1">{rate.suffix || '₽'}</span>
                    </div>
                  </div>
                );
              })
        ...
        {/* Calculator Section */}
        <section className="bg-gradient-to-b from-zinc-900/80 to-zinc-950/80 border border-zinc-800 rounded-[32px] p-5 sm:p-6 lg:p-8 backdrop-blur-sm relative overflow-hidden">
          {/* subtle glow */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
            ...
            <div className="md:col-span-5 flex flex-col gap-4 relative z-10">
              <div className="flex flex-col gap-2">
                <label className="text-[12px] font-medium text-zinc-400">Направление перевода</label>
              ...
              <div className="flex flex-col gap-2">
                <label className="text-[12px] font-medium text-zinc-400">Сумма к отправке</label>
                <div className="relative">
                  <input 
                    type="number"
                    value={calcAmount}
                    onChange={(e) => setCalcAmount(e.target.value)}
                    className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 sm:py-4 text-xl sm:text-2xl font-bold text-white placeholder-zinc-600 focus:outline-none focus:border-indigo-500/50 focus:ring-1 focus:ring-indigo-500/50 transition-all font-mono"
                    placeholder="1000"
                  />
                  <div className="absolute right-4 top-1/2 -translate-y-1/2 text-zinc-500 font-bold uppercase tracking-wider">
            ...
            {/* Results Area */}
            <div className="md:col-span-7 md:pl-6 md:border-l border-zinc-800/50 flex flex-col justify-center min-h-[160px]">
              <div className="space-y-4">
                
                <div className="flex flex-col gap-1">
                  <span className="text-[13px] text-zinc-400 font-medium">Итого к оплате (с комиссией):</span>
                  <div className="flex items-baseline gap-2">
                    <span className="text-4xl sm:text-5xl font-black tracking-tight text-white drop-shadow-md">
                      {new Intl.NumberFormat('ru-RU', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(getCalculationResult())}
                    </span>
                    <span className="text-xl sm:text-2xl font-bold text-zinc-500">₽</span>
                  </div>
                </div>

                {ratesData && (
                  <div className="flex items-center gap-4 text-[11px] sm:text-[12px] text-zinc-500 bg-zinc-900/50 inline-flex px-4 py-2 rounded-lg border border-zinc-800/50">
                    <div className="flex flex-col gap-0.5">
                      <span className="text-zinc-600">Курс валюты</span>
                    ...
                    <div className="flex flex-col gap-0.5">
                      <span className="text-zinc-600">Комиссия (эквивалент 100 {calcCurrency === "RUB/USD SWIFT" ? "USD" : "EUR"})</span>
                      <span className="font-medium text-zinc-400">
